...
setMat4 :: proc(shaderProgram : u32, name : cstring, mat4: ^glm.mat4) {
		// modelLoc := gl.GetUniformLocation(shaderProgram, name)
		gl.UniformMatrix4fv(
...
loadShader :: proc(vertex_source : ^cstring, fragment_source : ^cstring) -> u32 {
	...
	if success == 0 {
		gl.GetShaderInfoLog(vertex_shader, 512, nil, raw_data(infolog[:]))
	...
	if success == 0 {
		gl.GetShaderInfoLog(fragment_shader, 512, nil, raw_data(infolog[:]))
	...
	if success == 0 {
		gl.GetShaderInfoLog(shaderProgram, 512, nil, raw_data(infolog[:]))
